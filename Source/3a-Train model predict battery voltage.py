...
# MAGIC %run ./init/setuphack

# COMMAND ----------

# MAGIC %md
# MAGIC # XGBoost Regressor training
# MAGIC - Attach this notebook to a cluster with runtime version **12.x.x-cpu-ml-scala2.12**, and run it.
...
# We want to predict internalBatteryVoltage
target_col = "internalBatteryVoltage"
...
input_data_path = mlflow.artifacts.download_artifacts(run_id="c0d7e816391846e499c488cac6f5f591", artifact_path="data", dst_path=input_temp_dir)
...
supported_cols = ["engineRPM", "XAccelerometer", "modelName", "eventTS", "country", "vehicleSpeed"]
...
bool_transformers = [("boolean", bool_pipeline, ["modelName"])]
...
num_imputers = []
num_imputers.append(("impute_mean", SimpleImputer(), ["XAccelerometer", "engineRPM", "vehicleSpeed"]))
...
numerical_transformers = [("numerical", numerical_pipeline, ["engineRPM", "vehicleSpeed", "XAccelerometer"])]
...
categorical_one_hot_transformers = [("onehot", one_hot_pipeline, ["country"])]
...
# MAGIC 
# MAGIC `_automl_split_col_d395` contains the information of which set a given row belongs to.
...
# AutoML completed train - validation - test split internally and used _automl_split_col_d395 to specify the set
split_train_df = df_loaded.loc[df_loaded._automl_split_col_d395 == "train"]
split_val_df = df_loaded.loc[df_loaded._automl_split_col_d395 == "val"]
split_test_df = df_loaded.loc[df_loaded._automl_split_col_d395 == "test"]

# Separate target column from features and drop _automl_split_col_d395
X_train = split_train_df.drop([target_col, "_automl_split_col_d395"], axis=1)
y_train = split_train_df[target_col]

X_val = split_val_df.drop([target_col, "_automl_split_col_d395"], axis=1)
y_val = split_val_df[target_col]

X_test = split_test_df.drop([target_col, "_automl_split_col_d395"], axis=1)
...
# MAGIC - All the runs are logged under [this MLflow experiment](#mlflow/experiments/2556006740225425)
...
def objective(params):
  with mlflow.start_run(experiment_id="2556006740225425") as mlflow_run:
...
space = {
  "colsample_bytree": 0.6858156156501292,
  "learning_rate": 0.0780903987436257,
  "max_depth": 10,
  "min_child_weight": 1,
  "n_estimators": 1676,
  "n_jobs": 100,
  "subsample": 0.27903775035059547,
  "verbosity": 0,
  "random_state": 148298670,
}
...
if shap_enabled:
    ...
    train_sample = X_train.sample(n=min(100, X_train.shape[0]), random_state=148298670).fillna(mode)

    # Sample some rows from the validation set to explain. Increase the sample size for more thorough results.
    example = X_val.sample(n=min(100, X_val.shape[0]), random_state=148298670).fillna(mode)
...
# Register our model in the model registry using a name
model_name = f"PredictBatteryVoltage-{teamName}"
print(f"\nModel name to register: PredictBatteryVoltage-{teamName}")
